...
const {
  ...
  DataUpdate,
  DataDelete,
} = require("../middleware/databse_query");
...
router.get("/web_map_data", async (req, res) => {
  try {
    ...
    const gs = await DataFind(
      `SELECT google_map_key FROM tbl_general_settings`
    );

    await axios
      .get(
        `https://maps.googleapis.com/maps/api/place/textsearch/json?query=${text}&key=${gs[0].google_map_key}`
      )
      .then((response) => {
...
router.post("/signup", async (req, res) => {
  try {
    ...
    const missingField = ["name", "email", "ccode", "phone", "password"].find(
    ...
    const cus_data = await DataFind(
      `SELECT * FROM tbl_customer WHERE country_code = '${ccode}' AND phone = '${phone}'`
    );
    const general = await DataFind(
      `SELECT refer_credit, one_app_id, one_api_key FROM tbl_general_settings`
    );
    let generald = {
      one_app_id: general[0].one_app_id,
      one_api_key: general[0].one_api_key,
    };
    if (cus_data != "")
      return res
        .status(200)
    ...
    // Refer Cradit
    if (referral_code != "") {
      const referal_customer = await DataFind(
        `SELECT id, wallet FROM tbl_customer WHERE referral_code = '${referral_code}'`
      );
      if (referal_customer != "") {
        let amount =
          parseFloat(referal_customer[0].wallet) +
          parseFloat(general[0].refer_credit);
        if (
          (await DataUpdate(
            `tbl_customer`,
            `wallet = '${amount}'`,
            `id = '${referal_customer[0].id}'`,
            req.hostname,
            req.protocol
          )) == -1
        ) {
          return res
            .status(200)
            .json({ message: process.env.dataerror, status: false });
        }
        let fulldate = await AllFunction.TodatDate();
        if (
          (await DataInsert(
            `tbl_transaction_customer`,
            `c_id, payment_id, amount, date, status, type`,
            `${referal_customer[0].id}, '0', '${general[0].refer_credit}', '${fulldate.date}T${fulldate.time}', '1', ''`,
            req.hostname,
            req.protocol
          )) == -1
        ) {
    ...
    if (
      (await DataInsert(
        ...
        `'', ${esname}, '${email}', '${ccode}', '${phone}', '${hash}', '1', '${otp_result}', '0', '${date}'`,
        req.hostname,
        req.protocol
      )) == -1
    ) {
    ...
    const new_cus_data = await DataFind(
      `SELECT * FROM tbl_customer WHERE country_code = '${ccode}' AND phone = '${phone}'`
    );
    return res
      .status(200)
...
router.post(
  "/edit_customer",
  customer_profile.single("profile_img"),
  async (req, res) => {
    try {
      ...
      const missingField = ["id", "name", "email"].find(
        (field) => !req.body[field]
      );
      if (missingField) {
        return res
      ...
      let esname = mysql.escape(name),
        pass,
        imageUrl = "";
      const passw = await DataFind(
        `SELECT profile_image, password FROM tbl_customer WHERE id = '${id}'`
      );
      if (!password) pass = passw[0].password;
      else pass = await bcrypt.hash(password, 10);

      imageUrl = req.file
        ? "uploads/customer_profile/" + req.file.filename
        : passw[0].profile_image;
...
router.post("/login", async (req, res) => {
  try {
    ...
    const missingField = ["ccode", "phone", "password"].find(
      (field) => !req.body[field]
    );
    if (missingField)
      return res
    ...
    const general = await DataFind(
      `SELECT one_app_id, one_api_key FROM tbl_general_settings`
    );
    const cus_data = await DataFind(
      `SELECT * FROM tbl_customer WHERE country_code = '${ccode}' AND phone = '${phone}'`
    );
    ...
    if (cus_data == "")
      return res
        ...
        .json({
          ResponseCode: 401,
          Result: false,
          message: "PhoneNo Not Exists!",
          general: general[0],
        });

    if (cus_data[0].status != "1")
      return res
        .status(200)
        .json({
          ResponseCode: 401,
...
router.post("/mobile_check", async (req, res) => {
  try {
    ...
    if (cus_data == "")
      return res
        .status(200)
        .json({ ResponseCode: 200, Result: true, message: "New Number" });
    else
      return res
        .status(200)
        .json({
...
router.get("/otp_detail", async (req, res) => {
  try {
    const general_setting = await DataFind(
    ...
    if (sms_type != "")
      res
        .status(200)
        .json({ ResponseCode: 200, Result: true, message: sms_type });
    else
      res
        .status(200)
        .json({ ResponseCode: 401, Result: false, message: "Data Not Found" });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  }
...
router.post("/home", async (req, res) => {
  try {
    ...
    const general = await DataFind(
      `SELECT site_currency, offer_expire_time, google_map_key FROM tbl_general_settings`
    );
    if (general == "")
      return res
        .status(200)
    ...
    let vehicle_list = await DataFind(
      `SELECT id, image, name, description, bidding FROM tbl_vehicle WHERE status = '1' AND home_visible_status = '1'`
    );
    vehicle_list.map((vvel) => {
      vvel.role = "1";
    });

    const ms = await DataFind(
      `SELECT id, outstation, rental, package FROM tbl_general_settings`
    );
    ...
    let module_setting = [],
      package_list = [];
    if (midsArr.length > 0) {
      module_setting = await DataFind(
        `SELECT * FROM tbl_module_setting WHERE id IN (${midsArr})`
      );
    ...
    const cus_rating =
      await DataFind(`SELECT cus.id ${dr.tot_review} ${dr.avgstar}
                                        FROM tbl_customer AS cus
                                        ${dr.outtable}
                                        WHERE cus.id = '${uid}' GROUP BY cus.id ORDER BY id DESC`);

    let rdata =
      await DataFind(`SELECT veh.*, COALESCE(vec.minimum_fare, '') as minimum_fare, COALESCE(vec.maximum_fare, '') as maximum_fare
    ...
    if (rdata != "") {
      ...
      let pickapp = {
          title: rd.piclatlon.title,
          subtitle: rd.piclatlon.subtitle,
        },
        picklat = {
          latitude: rd.piclatlon.latitude,
          longitude: rd.piclatlon.longitude,
        };
      let drop_add = { title: "", subtitle: "" },
        drop_latlon = { latitude: "", longitude: "" },
        daddlist = [],
        dlatlist = [];
      for (let i = 0; i < rd.dropdata.length; ) {
        if (i == 0) {
          drop_add = {
            title: rd.dropdata[i].title,
            subtitle: rd.dropdata[i].subtitle,
          };
          drop_latlon = {
            latitude: rd.dropdata[i].latitude,
            longitude: rd.dropdata[i].longitude,
          };
        } else {
          daddlist.push({
            title: rd.dropdata[i].title,
      ...
      if (typeof rdata[0].d_id == "string")
        rdata[0].d_id = JSON.parse(rdata[0].d_id);
      let runtime = await AllFunction.CurrentDatetoOldDateS(
        rdata[0].start_time,
        general[0].offer_expire_time
      );

      rdata[0].pick_add = pickapp;
      rdata[0].pick_latlon = picklat;
      rdata[0].drop_add = drop_add;
      rdata[0].drop_latlon = drop_latlon;
      rdata[0].drop_add_list = daddlist;
      rdata[0].drop_latlon_list = dlatlist;
      rdata[0].bidding_run_status = rdata[0].bidding_d_price != "" ? 1 : 0;
      rdata[0].increased_time = runtime > 0 ? runtime : 0;
      rdata[0].price = Number(rdata[0].price);
      rdata[0].tot_km = Number(rdata[0].tot_km);

      delete rdata[0].pic_lat_long;
      delete rdata[0].drop_lat_long;
      delete rdata[0].pic_address;
      delete rdata[0].drop_address;
      delete rdata[0].status_time_location;
...
router.post("/vehicle_information", async (req, res) => {
  try {
    ...
    const vehicle_list = await DataFind(
      `SELECT id, image, name, description, passenger_capacity FROM tbl_vehicle WHERE id = '${vehicle_id}' AND status = '1'`
    );
...
router.post("/home_wallet", async (req, res) => {
  try {
    ...
    const driver = await DataFind(
      `SELECT wallet FROM tbl_customer WHERE id = '${uid}'`
    );
    if (driver == "")
      return res
        .status(200)
...
router.post("/home_mape", async (req, res) => {
  try {
    ...
    const general = await DataFind(
      `SELECT site_currency, vehicle_radius FROM tbl_general_settings`
    );
    ...
    const driver =
      await DataFind(`SELECT dr.id, COALESCE(ve.map_img, '') AS image, COALESCE(ve.name, '') AS name, COALESCE(ve.description, '') AS description, 
                                        ...
                                        WHERE 
                                        (6371 * ACOS(
                                            COS(RADIANS(dr.latitude)) * COS(RADIANS(${Number(
                                              lat
                                            )})) *
                                            COS(RADIANS(${Number(
                                              lon
                                            )}) - RADIANS(dr.longitude)) +
                                            SIN(RADIANS(dr.latitude)) * SIN(RADIANS(${Number(
                                              lat
                                            )}))
                                        )) <= ${Number(
                                          general[0].vehicle_radius
                                        )}
                                        AND JSON_CONTAINS(dr.zone, '${JSON.stringify(
                                          zonecheck[2].zid[0]
                                        )}', '$') AND dr.fstatus = '1' AND dr.status = '1' AND dr.approval_status = '1' AND dr.latitude NOT IN ('') AND dr.longitude NOT IN ('')`);
...
router.post("/calculate", async (req, res) => {
  try {
    const { uid, mid, mrole, pickup_lat_lon, drop_lat_lon, drop_lat_lon_list } =
      req.body;
    ...
    if (mrole == "1") {
      let vehicle = await DataFind(
        `SELECT * FROM tbl_vehicle WHERE status = '1' AND id = '${mid}'`
      );
      if (vehicle == "")
        return res
          .status(200)
      ...
      let convertzone = await AllFunction.ZoneLatlon(
        pickup_lat_lon,
        drop_lat_lon,
        drop_lat_lon_list
      );
      ...
      if (zonecheck[1].zc != 0)
        return res
          ...
          .json({
            ResponseCode: 401,
            Result: false,
            message: "Address is not in the zone!",
          });
      ...
      for (let c = 1; c < zoneresult.length; ) {
        if (zoneresult[c].status == "1") {
          let pickup = `${convertzone[c - 1].latitude},${
            convertzone[c - 1].longitude
          }`;
          let drop = `${convertzone[c].latitude},${convertzone[c].longitude}`;
          let distance = await AllFunction.GetDistance(
            pickup,
            drop,
            general[0].google_map_key
          );
      ...
      if (roundkm <= vehicle[0].min_km_distance) {
        ...
        add_distance =
          parseFloat(roundkm) - parseFloat(Number(vehicle[0].min_km_distance));
        km_charge =
          parseFloat(vehicle[0].min_km_distance) *
            parseFloat(vehicle[0].min_km_price) +
      ...
      if (general[0].weather_type == "fix") {
        ...
        const weatherp = Number(
          ((tot_price / 100) * Number(general[0].weather_price)).toFixed(2)
        );
        tot_price = tot_price + weatherp;
        weather_charge = weatherp;
      }
      ...
      if (vehicle[0].comission_type == "fix") {
        ...
        const scom = Number(
          ((tot_price / 100) * Number(vehicle[0].comission_rate)).toFixed(2)
        );
        tot_price = tot_price + scom;
        site_commission = scom;
      }
        ...
        if (Number(vehicle[0].minimum_fare) > Number(tot_price)) {
          return res
            ...
            .json({
              ResponseCode: 401,
              Result: false,
              message: `The fare in this city exceeds our minimum limit of ${general[0].site_currency}${vehicle[0].minimum_fare}.`,
            });
          ...
          return res
            ...
            .json({
              ResponseCode: 401,
              Result: false,
              message: `The fare in this city exceeds our maximum limit of ${general[0].site_currency}${vehicle[0].maximum_fare}.`,
            });
        } else {
    ...
    return res
      ...
      .json({
        ResponseCode: 401,
        Result: false,
        message: "Please try different type vehicle.!",
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
...
router.post("/module_calculate", async (req, res) => {
  try {
    ...
    const vehicle = await DataFind(
      `SELECT * FROM tbl_vehicle WHERE status = '1' AND bidding = '0' AND home_visible_status = '1'`
    );
    if (vehicle == "")
      return res
        .status(200)
    ...
    for (let a = 1; a < zoneresult.length; ) {
      let pickup = `${convertzone[a - 1].latitude},${
        convertzone[a - 1].longitude
      }`;
      ...
      if (spltime.length == "2") {
        dhour = 0;
        dtime = parseFloat(spltime[0]);
      } else if (spltime.length == "4") {
        dhour = parseFloat(spltime[0]);
        dtime = parseFloat(spltime[2]);
      }
    ...
    let hou_min = await AllFunction.MinuteToHour(totmin);
    let caldriver = [],
      cpickup = `${convertzone[0].latitude},${convertzone[0].longitude}`;
    for (let c = 0; c < vehicle.length; ) {
      ...
      const driver =
        await DataFind(`SELECT dr.id, COALESCE(ve.map_img, '') AS image, COALESCE(ve.name, '') AS name, COALESCE(ve.description, '') AS description,
                                            ...
                                            (
                                                6371 * ACOS(
                                                    COS(RADIANS(${Number(
                                                      convertzone[0].latitude
                                                    )})) * COS(RADIANS(dr.latitude)) *
                                                    COS(RADIANS(dr.longitude) - RADIANS(${Number(
                                                      convertzone[0].longitude
                                                    )})) +
                                                    SIN(RADIANS(${Number(
                                                      convertzone[0].latitude
      ...
      if (driver.length > 0) {
        ...
        let dstance = await AllFunction.GetDistance(
          driverl,
          cpickup,
          general[0].google_map_key
        );
        ...
        addtime = await AllFunction.AddDateMinute(
          parseFloat(dtime) + parseFloat(totmin)
        );
        veh = dtime;
      }
      if (cal <= vehicle[c].min_km_distance) {
        ...
        add_distance =
          parseFloat(cal) - parseFloat(Number(vehicle[c].min_km_distance));
        km_charge =
          parseFloat(vehicle[c].min_km_distance) *
            parseFloat(vehicle[0].min_km_price) +
          parseFloat(add_distance) * parseFloat(vehicle[0].after_km_price);
      }
      tot_price += km_charge;
      ...
      if (vehicle[c].comission_type == "fix") {
        ...
        const scom = Number(
          ((tot_price / 100) * Number(vehicle[c].comission_rate)).toFixed(2)
        );
        tot_price = tot_price + scom;
        site_commission = scom;
      }
      ...
      caldriver.push({
        ...
        m_role: mrole,
        dri_pic_time: veh,
        dri_pic_drop: addtime,
        drop_hour: hou_min.hour,
        drop_time: hou_min.minute,
        drop_km: Number(Number(cal).toFixed(2)),
        drop_price: Number(Number(tot_price).toFixed(2)),
        km_charge,
        extra_charge,
        weather_charge,
        extra_person_charge,
...
async function Vehicle_calculate(
  uid,
  mid,
  pickup_lat_lon,
  drop_lat_lon,
  ...
  if (status == 1) {
    convertzone = await AllFunction.ZoneLatlon(
      pickup_lat_lon,
      drop_lat_lon,
      drop_lat_lon_list
    );
    ...
    if (mid == "") {
      let vehicle_list = await DataFind(
        `SELECT id, image, name, description, bidding FROM tbl_vehicle WHERE status = '1'`
      );
      v_id = vehicle_list[0].id;
    } else v_id = mid;
  } else {
  ...
  const driver =
    await DataFind(`SELECT dr.id, COALESCE(ve.map_img, '') AS image, COALESCE(ve.name, '') AS name, COALESCE(ve.description, '') AS description, 
                                    ...
                                    AND dr.vehicle = '${v_id}' AND dr.zone IN (${
      zonecheck[2].zid
    }) AND dr.fstatus = '1' AND dr.status = '1' AND dr.approval_status = '1'
                                    AND dr.latitude NOT IN ('') AND dr.longitude NOT IN ('')`);
...
router.post("/vehicle_calculate", async (req, res) => {
  try {
    const { uid, mid, pickup_lat_lon, drop_lat_lon, drop_lat_lon_list } =
      req.body;

    const missingField = ["uid", "mid", "pickup_lat_lon", "drop_lat_lon"].find(
      (field) => !req.body[field]
    );
    if (missingField)
      return res
    ...
    const vehicle = await DataFind(
      `SELECT * FROM tbl_vehicle WHERE id = '${mid}' AND status = '1'`
    );
    if (vehicle == "")
      return res
        .status(200)
    ...
    const general = await DataFind(
      `SELECT vehicle_radius FROM tbl_general_settings`
    );
    if (general == "")
      return res
        .status(200)
...
router.post("/add_vehicle_request", async (req, res) => {
  try {
    ...
    const missingField = await AllFunction.BodyNumberDataCheck(
      [
        "uid",
        "vehicle_id",
        "tot_km",
    ...
    const general = await DataFind(
      `SELECT default_payment, vehicle_radius FROM tbl_general_settings`
    );
    let payment = payment_id == "0" ? general[0].default_payment : payment_id;

    let caldata = await AllFunction.AvailableDriverGet(
      convertz[0].latitude,
      convertz[0].longitude,
      general[0].vehicle_radius,
      `vehicle = '${vehicle_id}'`,
      zone,
      1
    );
    if (caldata.status == false)
      return res
        .status(200)
        .json({
          ResponseCode: 401,
          Result: false,
          message: "No drivers found nearby. Please try again shortly.!",
        });
    ...
    for (let i = 0; i < convertz.length; ) {
      if (i == "0") pic = convertz[i].latitude + "&!" + convertz[i].longitude;
        ...
        dropdata +=
          dropdata == ""
            ? convertz[i].latitude + "&!" + convertz[i].longitude
            : "&!!" + convertz[i].latitude + "&!" + convertz[i].longitude;
      alcount++;
      i++;
    }
    ...
    if (droplistadd && droplistadd != "") {
      for (let a = 0; a < droplistadd.length; ) {
        dropad +=
          dropad == ""
            ? droplistadd[a].title + "&!" + droplistadd[a].subt
            : "&!!" + droplistadd[a].title + "&!" + droplistadd[a].subt;
        drocount++;
        a++;
      }
    ...
    for (let i = 0; i < caldata.idlist.length; ) {
      sendOneNotification(
        "New ride request received! Ready to go!",
        "driver",
        caldata.idlist[i]
      );
      i++;
    ...
    let date = new Date().toISOString(),
      bidd_auto_st = bidd_auto_status == "true" ? "1" : "0";

    if (coupon_id != "") {
      const cl = await DataFind(
        `SELECT * FROM tbl_coupon WHERE id = '${coupon_id}'`
      );
      if (cl.length > 0) {
        if (
          new Date(date) >= new Date(cl[0].start_date) ||
          new Date(date) <= new Date(cl[0].end_date)
        ) {
          let totpc = drop_price - site_commission;
          if (totpc >= Number(cl[0].min_amount)) {
            drop_price -= Number(cl[0].discount_amount);
            coupon_price = Number(cl[0].discount_amount);
          }
        } else
          return res
            .status(200)
    ...
    let id = await DataInsert(
      `tbl_request_vehicle`,
      `c_id, d_id, vehicleid, bidding_d_price, bidding_status, bidd_auto_status, status, m_role, price, paid_amount, 
      ...
      `'${uid}', '${JSON.stringify(
        caldata.idlist
      )}', '${vehicle_id}', '', '${bidding}', '${bidd_auto_st}', '0', '1', '${
        bid_addjust_amount == "0" ? drop_price : bid_addjust_amount
      }', 
            '0', '${bidd_amount_dif}', '${km_charge}', '${extra_charge}', '${weather_charge}', '${site_commission}', '${extra_person_charge}', '0', '0', '0', '0', '0', '0', '0', '0', 
            '${coupon_id}', '${coupon_price}', '${payment}', '${tot_km}', '${tot_hour}', '${tot_minute}', '[]', '${JSON.stringify(
        zone
      )}', '', '${date}', '${pic}', '${dropdata}', 
            ${mysql.escape(picadd)}, ${mysql.escape(
        dropad
      )}, '', '', '0', '', ''`,
      req.hostname,
      req.protocol
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: `We’ve sent ${caldata.idlist.length} captain requests; they’ll confirm shortly.`,
        id: id.insertId,
...
router.get("/vehicle_cancel_reason", async (req, res) => {
  try {
    const ride_cancel_list = await DataFind(
      `SELECT id, title FROM tbl_ride_cancel_reason WHERE status = '1'`
    );
...
router.get("/review_data", async (req, res) => {
  try {
    const review_list = await DataFind(
      `SELECT * FROM tbl_ride_review_reason WHERE status = '1'`
    );
...
router.post("/vehicle_live_location", async (req, res) => {
  try {
    ...
    const data = await DataFind(
      `SELECT * FROM tbl_cart_vehicle WHERE c_id = '${uid}' AND d_id = '${d_id}'`
    );
    if (data == "")
      return res
        .status(200)
...
router.post("/timeout_vehicle_request", async (req, res) => {
  try {
    ...
    const data = await DataFind(
      `SELECT * FROM tbl_request_vehicle WHERE c_id = '${uid}'`
    );
    if (data == "")
      return res
        .status(200)
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: "Request Out successful",
        id: data[0].id,
        driverid: array,
      });
  } catch (error) {
...
router.post("/resend_vehicle_request", async (req, res) => {
  try {
    ...
    let pl = data[0].pic_lat_long.split("&!"),
      dl = data[0].drop_lat_long.split("&!!"),
      drop_lat_lon = "",
      drop_lat_lon_list = [];
    ...
    const general = await DataFind(
      `SELECT id, vehicle_radius FROM tbl_general_settings`
    );
    ...
    if (
      (await DataUpdate(
        `tbl_request_vehicle`,
        `d_id = '${senddataJson}', bidding_d_price = '', status = '0'`,
        `c_id = '${uid}'`,
        req.hostname,
        req.protocol
      )) == -1
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: "Request Resend successful",
        id: data[0].id,
        driver_list: JSON.parse(senddataJson),
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
...
router.post("/vehicle_ride_cancel", async (req, res) => {
  try {
    ...
    let rd,
      indata,
      fulldate = await AllFunction.TodatDate(),
      status = "4",
      driver_list = [];
    rd = await DataFind(
      `SELECT * FROM tbl_request_vehicle WHERE id = '${request_id}' AND c_id = '${uid}'`
    );
    if (rd == "") {
      rd = await DataFind(
        `SELECT * FROM tbl_cart_vehicle WHERE (id = '${request_id}' OR req_id = '${request_id}') AND c_id = '${uid}'`
      );
      if (rd == "")
        return res
          .status(200)
      ...
      let updatet =
        rd[0].status_time_location != ""
          ? `${rd[0].status_time_location}&!!${status}&${fulldate.date}T${fulldate.time}&${lat}&${lon}`
      ...
      let stime = setime[0].split("&"),
        etime = new Date().toISOString();
      if (stime[0] == "1") starttime = await AllFunction.TodatDate(stime[1]);
      const packd =
        typeof rd[0].package_details == "string"
          ? JSON.parse(rd[0].package_details)
          : rd[0].package_details;
      const jzone =
        typeof rd[0].zone == "string" ? JSON.parse(rd[0].zone) : rd[0].zone;
      indata = await DataInsert(
        `tbl_order_vehicle`,
        ...
        `'${rd[0].c_id}', '${rd[0].d_id}', '${rd[0].vehicleid}', '${
          rd[0].bidding_status
        }', '${rd[0].bidd_auto_status}', '4', '${rd[0].m_role}', '0', '${
          rd[0].price
        }',
                '${rd[0].final_price}', '0', '${rd[0].bid_addjust_amount}', '${
          rd[0].km_charge
        }', '${rd[0].additional_time}', '${rd[0].addi_time_price}', '${
          rd[0].platform_fee
        }', '${rd[0].weather_price}',
                '${rd[0].coupon_id}', '${rd[0].coupon_amount}', '0', '${
          rd[0].extra_charge
        }', '${rd[0].extra_person_charge}', '${rd[0].day_charge}', '${
          rd[0].rent_hour
        }',
                '${rd[0].rental_addi_ride_time}', '${
          rd[0].rental_addi_ride_charge
        }', '${rd[0].rental_hour_charge}', '${
          rd[0].rental_per_hour_discount
        }', '${rd[0].rental_extra_km}',
                '${rd[0].rental_extra_km_charge}', '${rd[0].tot_kg}', '${
          rd[0].kg_charge
        }', '${rd[0].payment_id}', '${rd[0].tot_km}', '${rd[0].tot_hour}', '${
          rd[0].tot_minute
        }',
                '${JSON.stringify(packd)}', '${JSON.stringify(jzone)}', '${
          rd[0].ride_status
        }', '${stime[1]}', '${etime}', '${rd[0].drop_tot}', '${
          rd[0].drop_complete
        }',
                '${rd[0].current_run_time}', '${
          rd[0].status_time
        }', '${updatet}', '', '${rd[0].pic_lat_long}', '${
          rd[0].drop_lat_long
        }', ${mysql.escape(rd[0].pic_address)},
                ${mysql.escape(rd[0].drop_address)}, '', '${cancel_id}', '${
          rd[0].outs_category_id
        }', '${rd[0].mod_cate_id}', '${rd[0].num_passenger}', '${
          rd[0].book_date
        }',
                '${rd[0].book_time}', '${rd[0].id}'`,
        req.hostname,
        req.protocol
      );
      if (indata == -1)
      ...
      if (
        (await DataUpdate(
          `tbl_driver`,
          `rid_status = '0' AND check_status = '0'`,
          `id = '${rd[0].d_id}'`,
          req.hostname,
          req.protocol
        )) == -1
      ...
      indata = await DataInsert(
        ...
        `'${rd[0].c_id}', '${rd[0].d_id}', '${rd[0].vehicleid}', '${
          ...
          rd[0].price
                ...
                '0', '0', '${rd[0].bid_addjust_amount}', '${
          rd[0].km_charge
        }', '0', '0', '${rd[0].platform_fee}', '${rd[0].weather_price}', '${
          rd[0].coupon_id
        }', '${rd[0].coupon_amount}', '0', '${rd[0].extra_charge}',
                '${rd[0].extra_person_charge}', '${rd[0].day_charge}', '${
          rd[0].rent_hour
        }', '${rd[0].rental_addi_ride_time}', '${
          rd[0].rental_addi_ride_charge
          ...
          rd[0].kg_charge
                ...
                '${rd[0].tot_km}', '${rd[0].tot_hour}', '${
          rd[0].tot_minute
        }', '${JSON.stringify(packd)}', '${JSON.stringify(
          jzone
        )}', '0', '', '', '${dropcount}', '', '', '',
                '${updatet}', '', '${rd[0].pic_lat_long}', '${
          rd[0].drop_lat_long
        }', ${mysql.escape(rd[0].pic_address)}, ${mysql.escape(
          rd[0].drop_address
        )}, '', '${cancel_id}',
                '${rd[0].outs_category_id}', '${rd[0].mod_cate_id}', '${
          rd[0].num_passenger
        }', '${rd[0].book_date}', '${rd[0].book_time}', '${rd[0].id}'`,
        req.hostname,
        req.protocol
      );
      if (indata == -1)
...
router.post("/vehicle_ride_addstatus", async (req, res) => {
  try {
    ...
    const rd = await DataFind(
      `SELECT * FROM tbl_cart_vehicle WHERE id = '${request_id}' AND c_id = '${uid}' AND d_id = '${d_id}'`
    );
    if (rd == "")
      return res
        .status(200)
    ...
    if (rd[0].status == "3") {
      if (rd[0].ride_status != "0")
        return res
          .status(200)
          .json({
            ResponseCode: 401,
            Result: false,
            message: "Please Complete Other Stap!",
          });
    } else if (rd[0].status == "6") {
      if (rd[0].ride_status != "6")
        return res
          .status(200)
          .json({
            ResponseCode: 401,
    ...
    let dlatlon = rd[0].drop_lat_long.split("&!!"),
      daddress = rd[0].drop_address.split("&!!"),
      pending_ride = "",
      drop_list = [];
    ...
    let ccheck = await AllFunction.CheckCurrentLocation(
      pending_ride,
      dlatlon,
      daddress
    );
    ...
    for (let i = 0; i < dlatlon.length; ) {
      let checkadd = daddress[i].split("&!"),
        checkl = dlatlon[i].split("&!"),
        status = "";
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: "Service Start Successful",
        current_address: ccheck.current_address,
        next_address: ccheck.next_address,
        drop_list,
      });
  } catch (error) {
    console.error(error);
...
router.post("/vehicle_driver_detail", async (req, res) => {
  try {
    ...
    let rd =
      await DataFind(`SELECT cve.id, cve.c_id, cve.d_id, cve.price, cve.tot_km, cve.status, cve.otp, cve.tot_hour, cve.tot_minute,
                                ...
                                ${dr.tot_review} ${
        dr.avgstar
      }, COALESCE(COUNT(DISTINCT ov.id), 0) AS tot_complete_order
                                ...
                                WHERE ${
                                  request_id != ""
                                    ? `cve.id = '${request_id}' AND`
                                    : ``
                                }  cve.c_id = '${uid}' AND cve.d_id = '${d_id}' GROUP BY cve.id`);

    if (rd != "") {
      rd[0].rating = parseFloat(rd[0].avg_star);
      rd[0].tot_review = parseFloat(rd[0].tot_review);
      rd[0].tot_hour = parseFloat(rd[0].tot_hour);
      rd[0].tot_minute = parseFloat(rd[0].tot_minute);
      ...
      return res
        ...
        .json({
          ResponseCode: 200,
          Result: true,
          message: "Driver Data Load Successful",
          accepted_d_detail: rd[0],
        });
    } else {
      return res
        .status(200)
...
router.post("/driver_profile_detail", async (req, res) => {
  try {
    ...
    let rd =
      await DataFind(`SELECT dr.profile_image, dr.vehicle_image, dr.first_name, dr.last_name, dr.primary_ccode, dr.primary_phoneNo, dr.language, dr.vehicle_number, dr.car_color,
...
router.post(
  ...
  async (req, res) => {
    try {
      ...
      const rd = await DataFind(
        `SELECT * FROM tbl_cart_vehicle WHERE (id = '${request_id}' OR req_id = '${request_id}') AND c_id = '${uid}' AND d_id = '${d_id}'`
      );
      if (rd.length == 0)
        return res
          .status(200)
      ...
      const dr = await DataFind(
        `SELECT * FROM tbl_driver WHERE id = '${rd[0].d_id}'`
      );
      if (dr.length == 0)
        return res
          .status(200)
      ...
      const customer = await DataFind(
        `SELECT * FROM tbl_customer WHERE status = 1 AND id = '${uid}'`
      );
      if (customer == "")
        return res
          .status(200)
      ...
      let setime = rd[0].status_time_location.split("&!!"),
        ...
        paid_status = 0;
      let stime = setime[0].split("&"),
        etime = setime[setime.length - 1].split("&");
      ...
      paid_amount = 0;
      (paid_status = 1),
        (fullwallet = parseFloat(dr[0].wallet)),
        (cash = 0),
        (tot_cash = 0),
        (payout_wallet = 0),
        (fulldate = await AllFunction.TodatDate());
      // Add Driver Amount
      const nwallet_amount = parseFloat(
        (
          parseFloat(rd[0].price) +
          parseFloat(rd[0].addi_time_price) +
          parseFloat(rd[0].weather_price)
        ).toFixed(2)
      );
      ...
      if (Number(wallet) > 0) {
        ...
        if (Number(rd[0].final_price) > Number(wallet)) {
          paid_amount = parseFloat(
            (parseFloat(rd[0].final_price) - parseFloat(wallet)).toFixed(2)
          );
          // paid_amount = Online Amount
          if (
            parseFloat(rd[0].platform_fee) > parseFloat(paid_amount) &&
            payment_id == "9"
          ) {
            cash = parseFloat(
              (
        ...
        let cdata = customer[0].wallet,
          wamount = 0;
        console.log(cdata || cdata != "0" || cdata != "NaN");
        console.log(cdata);

        if (cdata || cdata != "0" || cdata != "NaN") wamount = cdata;
        let wallet_amount = parseFloat(
          (parseFloat(wamount) - parseFloat(wallet)).toFixed(2)
        );
        ...
        if (payment_id == "9") {
          fullwallet = parseFloat(
            (parseFloat(dr[0].wallet) + parseFloat(nwallet_amount)).toFixed(2)
          );
          payout_wallet = parseFloat(
            (parseFloat(dr[0].payout_wallet) + parseFloat(wallet)).toFixed(2)
          );
        }
      } else {
      ...
      if (payment_id == "9") {
        ...
        if (cash == 0)
          tot_cash = parseFloat(
            (
              parseFloat(dr[0].tot_cash) + parseFloat(rd[0].platform_fee)
            ).toFixed(2)
          );
        else
          tot_cash = parseFloat(
            (parseFloat(dr[0].tot_cash) + parseFloat(cash)).toFixed(2)
          );
      } else {
        payout_wallet = parseFloat(
          (
            parseFloat(dr[0].payout_wallet) + parseFloat(nwallet_amount)
          ).toFixed(2)
        );
      ...
      if (
        (await DataUpdate(
          `tbl_driver`,
          `wallet = '${fullwallet}', payout_wallet = '${payout_wallet}', tot_cash = '${tot_cash}', rid_status = '0' AND check_status = '0'`,
          `id = '${rd[0].d_id}'`,
          req.hostname,
          req.protocol
        )) == -1
      ...
      let insertid = await DataInsert(
        ...
        `'${rd[0].c_id}', '${rd[0].d_id}', '${rd[0].vehicleid}', '${
          rd[0].bidding_status
        }', '${rd[0].bidd_auto_status}', '8', '${
          rd[0].m_role
        }', '${paid_status}', '${rd[0].price}', 
            '${rd[0].final_price}', '${paid_amount}', '${
          rd[0].bid_addjust_amount
        }', '${rd[0].km_charge}', '${rd[0].additional_time}', '${
          rd[0].addi_time_price
        }', '${rd[0].platform_fee}', '${rd[0].weather_price}', 
            '${rd[0].coupon_id}', '${rd[0].coupon_amount}', '${wallet}', '${
          rd[0].extra_charge
        }', '${rd[0].extra_person_charge}', '${rd[0].day_charge}', '${
          rd[0].rent_hour
        }', '${rd[0].rental_addi_ride_time}', 
            '${rd[0].rental_addi_ride_charge}', '${rd[0].rental_hour_charge}', 
            '${rd[0].rental_per_hour_discount}', '${rd[0].rental_extra_km}', '${
          rd[0].rental_extra_km_charge
        }', '${rd[0].tot_kg}', '${rd[0].kg_charge}', '${rd[0].payment_id}', 
            '${rd[0].tot_km}', '${rd[0].tot_hour}', '${
          rd[0].tot_minute
        }', '${JSON.stringify(packd)}', '${JSON.stringify(jzone)}', '${
          rd[0].ride_status
        }', '${stime[1]}', '${etime[1]}', 
            '${rd[0].drop_tot}', '${rd[0].drop_complete}', '${
          rd[0].current_run_time
        }', '${rd[0].status_time}', '${updatet}', '${
          rd[0].status_calculation
        }', '${rd[0].pic_lat_long}', 
            '${rd[0].drop_lat_long}', ${mysql.escape(
          rd[0].pic_address
        )}, ${mysql.escape(rd[0].drop_address)}, '${imageUrl}', '', '${
          rd[0].outs_category_id
        }', '${rd[0].mod_cate_id}', '${rd[0].num_passenger}', '${
          rd[0].book_date
        }', 
...
router.post("/tbl_review", async (req, res) => {
  try {
    ...
    if (!uid || !d_id || !request_id || !tot_star)
      return res
        .status(200)
        .json({
          ResponseCode: 401,
    ...
    const rd = await DataFind(
      `SELECT * FROM tbl_order_vehicle WHERE id = '${request_id}' AND c_id = '${uid}' AND d_id = '${d_id}'`
    );
    if (rd == "")
      return res
        .status(200)
    ...
    const revi = await DataFind(
      `SELECT * FROM tbl_review_driver WHERE request_id = '${rd[0].id}' AND customer_id = '${uid}' AND driver_id = '${d_id}'`
    );
    if (revi != "")
      return res
        .status(200)
        .json({ ResponseCode: 401, Result: false, message: "Review Found!" });

    let esreview = mysql.escape(review),
      fulldate = new Date().toISOString();
    if (
      (await DataInsert(
        `tbl_review_driver`,
        `driver_id, customer_id, request_id, def_review, review, tot_star, date`,
        `'${d_id}', '${uid}', '${rd[0].id}', '${def_review}', ${esreview}, '${tot_star}', '${fulldate}'`,
        req.hostname,
        req.protocol
      )) == -1
    ) {
...
async function AllServiceData(status, where, tblname, field, otp) {
  ...
  if (status == "1") {
    data =
      await DataFind(`SELECT cv.id, cv.d_id, cv.price, cv.status, cv.bid_addjust_amount, cv.km_charge, cv.extra_charge, cv.extra_person_charge, cv.day_charge, cv.rent_hour, cv.rental_hour_charge, 
    ...
    data =
      await DataFind(`SELECT cv.id, cv.d_id, cv.price, cv.status, cv.price, cv.final_price, cv.paid_amount, cv.bid_addjust_amount, cv.coupon_amount, cv.addi_time_price, cv.platform_fee, 
...
router.post("/all_service_request", async (req, res) => {
  try {
    ...
    const customer = await DataFind(
      `SELECT * FROM tbl_customer WHERE id = '${uid}'`
    );
    if (customer == "")
      return res
        .status(200)
    ...
    const general = await DataFind(
      `SELECT driver_wait_time FROM tbl_general_settings`
    );
    if (general == "")
      return res
        .status(200)
    ...
    if (status == "upcoming") {
      let pen_req_request = await AllServiceData(
        1,
        `WHERE cv.c_id = '${uid}'`,
        `tbl_request_vehicle`,
        0
      ...
      let first_list = pen_req_request.map(async (val) => {
        ...
        let otherdata = {
          ...
          additional_time: 0,
          coupon_id: "",
          rental_addi_ride_time: 0,
          rental_addi_ride_charge: 0,
          current_run_time: "",
          profile_image: "",
          vehicle_image: "",
          first_name: "",
      ...
      let pen_card_request = await AllServiceData(
        2,
        `WHERE cv.c_id = '${uid}'`,
        `tbl_cart_vehicle`,
        ", cv.driver_id_list",
      ...
      totrequest = await AllServiceData(
        2,
        `WHERE cv.c_id = '${uid}' AND cv.status = '8'`,
        `tbl_order_vehicle`,
        "",
        ""
      );
      ...
      totrequest = await AllServiceData(
        2,
        `WHERE cv.c_id = '${uid}' AND cv.status = '4'`,
        `tbl_order_vehicle`,
        "",
        ""
      );
    ...
    if (totrequest != "") {
      ...
      return res
        ...
        .json({
          ...
          Result: true,
          message: "Data Load Successful",
          driver_wait_time: parseFloat(general[0].driver_wait_time),
          Reuqest_list: Reuqest_list,
        });
    } else {
      return res
        .status(200)
...
router.post("/all_ride_detail", async (req, res) => {
  try {
    ...
    if (status == "complete") {
      ...
      let mapdata =
        await DataFind(`SELECT cv.id, cv.c_id, cv.d_id, cv.price, cv.final_price, cv.paid_amount, cv.bid_addjust_amount, cv.coupon_amount, cv.addi_time_price, 
      ...
      if (mapdata != "") {
        const review =
          await DataFind(`SELECT * FROM tbl_review_driver WHERE request_id = '${mapdata[0].id}' AND driver_id = '${mapdata[0].d_id}' 
                                                AND customer_id = '${mapdata[0].c_id}'`);

        if (review != "") review_check = 1;
        mapdata[0].c_title = "";
        mapdata[0].addi_time = parseFloat(mapdata[0].additional_time);

        mapdata[0].price = parseFloat(mapdata[0].price);
        mapdata[0].tot_km = parseFloat(mapdata[0].tot_km);
        mapdata[0].review_check = review_check;
        mapdata[0].review_check = review_check;
        Reuqest_list = await AllFunction.AllVehicleFormate(mapdata, 2);

        delete Reuqest_list[0].tot_drop;
        delete Reuqest_list[0].comission_rate;
        delete Reuqest_list[0].comission_type;
        delete Reuqest_list[0].additional_time;
        return res
          .status(200)
          .json({
            ResponseCode: 200,
...
router.post("/ride_additional_detail", async (req, res) => {
  try {
    ...
    let check =
      await DataFind(`SELECT id, m_role, book_date, book_time, num_passenger, rent_hour, tot_kg, package_details FROM
                                    ${
                                      status == 1
                                        ? "tbl_cart_vehicle"
                                        : "tbl_order_vehicle"
    ...
    check[0].package_details =
      typeof check[0].package_details == "string"
        ? JSON.parse(check[0].package_details)
        : check[0].package_details;
    return res
      .status(200)
      .json({
        ...
        add_data: check.length > 0 ? check[0] : {},
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
...
router.get("/coupon_payment", async (req, res) => {
  try {
    let fulldate = await AllFunction.TodatDate();
    let coupon_list = await DataFind(
      `SELECT * FROM tbl_coupon WHERE start_date <= '${fulldate.date}' AND end_date >= '${fulldate.date}'`
    );
    ...
    let bank_data = [
      { bank_name: spldata[0] },
      { holder_name: spldata[1] },
      { account_no: spldata[2] },
      { iafc_code: spldata[3] },
      { swift_code: spldata[4] },
    ];

    const general = await DataFind(
      `SELECT default_payment FROM tbl_general_settings`
    );
    ...
    return res
      ...
      .json({
        ...
        Result: true,
        message: "Data Load successful",
        default_payment: general[0].default_payment,
        coupon_list,
        payment_list,
        bank_data,
      });
...
router.post("/all_data", async (req, res) => {
  try {
    ...
    let module_data = [];
    if (mrole == "2")
      module_data = await DataFind(
        `SELECT * FROM tbl_outstation WHERE status = 1`
      );

    if (mrole == "3")
      module_data = await DataFind(`SELECT * FROM tbl_rental WHERE status = 1`);
...
router.post("/module_data", async (req, res) => {
  try {
    ...
    if (mrole == "2")
      module_data = await DataFind(
        `SELECT * FROM tbl_outstation WHERE status = 1 AND id = '${mid}'`
      );

    if (mrole == "3")
      module_data = await DataFind(
        `SELECT * FROM tbl_rental WHERE status = 1 AND id = '${mid}'`
      );
...
router.post("/add_wallet", async (req, res) => {
  try {
    ...
    const missingField = await AllFunction.CheckBodyData(req, [
      "uid",
      "payment_id",
      "amount",
    ]);
    ...
    if (
      (await DataInsert(
        `tbl_transaction_customer`,
        `c_id, payment_id, amount, date, status, type`,
        `${uid}, '${payment_id}', '${amount}', '${fulldate.date}T${fulldate.time}', '1', ''`,
        req.hostname,
        req.protocol
      )) == -1
    ) {
...
router.post("/wallet", async (req, res) => {
  try {
    ...
    let wallet_data =
      await DataFind(`SELECT dt.*, COALESCE(pd.name, '') AS pname
    ...
    return res
      ...
      .json({
        ...
        Result: true,
        message: "Wallet Add successful",
        wallet_amount: customer[0].wallet,
        wallet_data: fulldata,
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
  ...
  try {
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: "Pages load successfully",
        pages_list,
      });
  } catch (error) {
    console.error(error);
...
router.post("/refer_and_earn", async (req, res) => {
  try {
    ...
    const customer = await DataFind(
      `SELECT id, referral_code FROM tbl_customer WHERE id = '${uid}'`
    );
    const general = await DataFind(
      `SELECT refer_credit, signup_credit FROM tbl_general_settings`
    );

    if (customer != "" || general != "") {
      let refer_data = {
        referral_code: customer[0].referral_code,
        refer_credit: general[0].refer_credit,
        signup_credit: general[0].signup_credit,
      };
      return res
        .status(200)
        .json({
...
router.post("/notification", async (req, res) => {
  try {
    ...
    const ndata =
      await DataFind(`SELECT noti.id, noti.image, noti.title, noti.description, noti.date
...
router.post("/account_deactive", async (req, res) => {
  try {
    ...
    const driver_list = await DataFind(
      `SELECT id FROM tbl_customer WHERE id = '${id}'`
    );
    if (driver_list == "")
      return res
        .status(200)
        .json({ ResponseCode: 401, Result: false, message: "ID Not Found!" });
    ...
    return res
      ...
      .json({
        ResponseCode: 200,
        Result: true,
        message: "Account Deactivate Successful",
      });
  } catch (error) {
    console.error(error);
    res.status(500).json({ error: "Internal server error" });
